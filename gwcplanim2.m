...
%    xindex=xindex(2:end-1);
    ...
    labelstr = {s(2:end-1).label};
    for labeln = 1:numel(labelstr)
        labelnstr = labelstr{labeln};
        labelstr(labeln) = {[labelnstr ',' num2str(labeln)]};
    end
    text( x(1,xindex)+skew(1),x(2,xindex)+skew(2), labelstr);
end
res =2;
