...
matcont_path = [results_path 'matcont wsnetworks_n=10k=2p=0.05number=796'];
...
fprintf('Network 对称轴 %f, 与选择的 n = %d.\n', ((d1-d2)*c*(1-c))/(2*d1*d2), network_p)
format rat
network_b_star
network_a_star
format long
...
[t,y]=ode45(hls{2},[0:1:5e4],X0',options,P0(1),P0(2),P0(3),P0(4),P0(5));
% size(t)
% size(y)
...
[x0,v0]=init_EP_EP(@RDPPSystemOnNetwork2,initial_x0,p,ap1);
opt=contset;
...
% plot(p(ap1),initial_x1(1),'k*')
% [x0,v0]=init_BP_EP(@RDPPSystemOnNetwork2,initial_x1,p,s4(BP_pos),0.01);
% opt=contset;
% opt=contset(opt,'VarTolerance',1e-3);
% opt=contset(opt,'FunTolerance',1e-3);
% opt=contset(opt,'MaxStepsize',0.01);
...
% % ap1=[2 1]
% % [x0,v0]=init_BP_LP(@RDPPSystemOnNetwork2,initial_x1,p,ap1);
% % opt=contset(opt,'InitStepsize',0.0001);
...
initial_x1=x2(1:2*N,s2(BP_pos).index);
p(ap1)=x2(2*N+1,s2(BP_pos).index);
[x0,v0]=init_BP_LP(@RDPPSystemOnNetwork2,initial_x1,p,[2 1]);
opt=contset;
opt=contset(opt,'VarTolerance',1e-3);
opt=contset(opt,'FunTolerance',1e-3);
opt=contset(opt,'MaxStepsize',0.01);
...
% ap1=[2 1];
% [x0,v0]=init_H_H(@RDPPSystemOnNetwork2,initial_xH,p,ap1);
% opt=contset;
% opt=contset(opt,'Singularities',1);
% opt=contset(opt,'MaxStepsize',0.01);
...
[x0,v0]=init_ZH_H(@RDPPSystemOnNetwork2,initial_xZH,p,ap1);
opt=contset;
opt=contset(opt,'VarTolerance',1e-3);
opt=contset(opt,'FunTolerance',1e-3);
opt=contset(opt,'MaxStepsize',0.001);
...
% [x0,v0]=init_ZH_LP(@RDPPSystemOnNetwork2,initial_xZH,p,ap1);
[x0,v0]=init_LP_LP(@RDPPSystemOnNetwork2,initial_xZH,p,ap1,[1 2]);
opt=contset;
