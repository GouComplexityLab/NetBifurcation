...
% which can be download from https://www.mathworks.com/matlabcentral/fileexchange/125140-bubble-digraph
    ...
    methods
        function obj=bubbleGraph_BrainGou(Data,Class,varargin)
            obj.Data=Data;
            obj.Class=Class(:);
        ...
        function obj=draw(obj)
            ...
            pian = 0.38;
            pian2 = 0.05;
            thetaList1=linspace(pi-pian*pi,pi+(pian+pian2)*pi,41);
            thetaList2=linspace(2*pi-(pian+pian2)*pi,2*pi+pian*pi,41);
            thetaList2=fliplr(thetaList2);

            thetaList = [thetaList1,thetaList2];

            XList=cos(thetaList);YList=sin(thetaList);
            aaa = sum(abs(obj.Data));
            size_data = ones(size(aaa));
            size_data(end) = 1;
            obj.bubbleHdl=bubblechart(XList,YList,size_data,'MarkerFaceAlpha',0.80);
