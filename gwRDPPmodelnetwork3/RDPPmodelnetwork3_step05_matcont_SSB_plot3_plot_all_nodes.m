...
figure_path = [results_path 'figure step03 ssb nodes wsnetworks_n=10k=2p=0.05number=69'];
...
plot3(right_para(1:5:end),Node_pos*ones(size(right_para(1:5:end))),right_unode(1:5:end),'Marker','none',...
    'LineWidth',gwlinewidth,'LineStyle','--','Color','k');
...
% plot(cont2_para,cont2_unode,'Marker','none',...
%     'LineWidth',gwlinewidth,'LineStyle','-','Color',gwcolors(Node_pos,:));
plot3(cont2_para,Node_pos*ones(size(cont2_para)),cont2_unode,'Marker','none',...
    'LineWidth',gwlinewidth,'LineStyle','-','Color',gwcolors(Node_pos,:));
...
xlim([b_star-0.00015 b_star+0.0001])
zlim([0.045 0.051])
ylim([0 11])
% view([75 30])
view([80 20])
...
% zticks = zticks(ax);
zticks = [0.046:0.002:0.051];
zticklabels = arrayfun(@(x) sprintf('%.3f' , x), zticks, 'UniformOutput', false);
% zticklabels(1) = {'0'};
% zticklabels(end) = {'$b$'};
