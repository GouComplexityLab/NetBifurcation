...
mu_up_right = linspace(mu2,0,1000);
delta = (Bk20 + Bk21*mu_up_right).^2 - 4*Bk11*Bk30*mu_up_right;
x_up_1 = (-(Bk20 + Bk21*mu_up_right) - sqrt(delta)) / 2/ Bk30;
plot(mu_up_right, x_up_1,'MarkerSize',8,'LineWidth',gwLineWidth,'LineStyle',':','Color','k');

mu_down_right1 = linspace(mu2,maxmu,1000);
delta = (Bk20 + Bk21*mu_down_right1).^2 - 4*Bk11*Bk30*mu_down_right1;
x_down_1 = (-(Bk20 + Bk21*mu_down_right1) + sqrt(delta)) / 2/ Bk30;
plot(mu_down_right1, x_down_1,'MarkerSize',8,'LineWidth',gwLineWidth,'LineStyle','-','Color','k');
...
% yticks = yticks(ax);
yticks = [-0.01,-0.005,0,0.005,max_y];
yticklabels = arrayfun(@(x) sprintf('%.3f', x), yticks*1, 'UniformOutput', false);
yticklabels(3) = {'0'};
% yticklabels(end) = {'$a$'};
...
figure_name = [results_path 'Figure Step02 SSBNF diagram wsnetworks_n=10k=2p=0.05number=69.eps'];
