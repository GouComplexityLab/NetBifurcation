...
[x0,v0]=init_EP_EP(@FHNSystemOnBrain,initial_x0,p,ap);
opt=contset;
...
% [x0,v0]=init_BP_EP(@FHNSystemOnBrain,initial_x1,p,s2(BP_pos),0.01);
% % BP_pos = 2;
% % initial_x1=x1(1:2*N,s1(BP_pos).index);
% % p(ap)=x1(2*N+1,s1(BP_pos).index);
% % [x0,v0]=init_BP_EP(@FHNSystemOnBrain,initial_x1,p,s1(BP_pos),0.01);
% opt=contset;
...
% [x0,v0]=init_H_LC(@FHNSystemOnBrain,initial_x1,p,[1],1e-6,20,4);
HP_pos = 2;
