...
% % Initial condition 
init_n = 1;
Initial_U = u_star*ones(size(x)) + 0.005*ones(size(x)) + 0.05 * special_eigenvector';
Initial_V = v_star*ones(size(x)) + 0.005*ones(size(x)) + 0.05 * special_eigenvector';
...
% Y0 = [Initial_U'; Initial_V'];
% T = 30000; Time_interval = 0.01;
% [time,solY1] = ode45(@NetworkRDsystem,[0:Time_interval:T],Y0);
% 
% Y0 = solY1(end,:);
% T = 3000; Time_interval = 0.01;
% [time,solY1] = ode45(@NetworkRDsystem,[0:Time_interval:T],Y0);
