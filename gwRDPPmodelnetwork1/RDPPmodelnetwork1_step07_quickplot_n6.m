...
figure_path = [results_path 'figure step05 THB series npbcwsnetworks_n=10k=2p=0number=1' '/region ' num2str(domain_n)];
...
for init_n = 1:4
    ...
    ylim([min(cU_series(:))-0.01 max(cU_series(:))+0.01])
    ...
    ylim([min(cV_series(:))-0.01 max(cV_series(:))+0.01])
    ...
    xlim_left = min(cU_series(:))-0.01;
    xlim_right = max(cU_series(:))+0.01;
    ylim_left = min(cV_series(:))-0.01;
    ylim_right = max(cV_series(:))+0.01;
