...
%%
matcont_path = [results_path 'matcont npbcwsnetworks_n=10k=2p=0number=1'];
...
X0(1:2:end-1) = u_star + eigenvectors(:,2)* 0.005;
X0(2:2:end  ) = v_star + eigenvectors(:,2)* 0.005;
...
[t,y]=ode45(hls{2},[0:0.1:10000],X0',options,P0(1),P0(2),P0(3),P0(4),P0(5));
size(t)
size(y)
figure
hold on
...
[x0,v0]=init_EP_EP(@RDPPSystemOnNetwork1,initial_x0,p,ap1);
opt=contset;
...
initial_x1=x2(1:2*N,s2(BP_pos).index);
p(ap1)=x2(2*N+1,s2(BP_pos).index);
[x0,v0]=init_BP_LP(@RDPPSystemOnNetwork1,initial_x1,p,[2 1]);
opt=contset;
opt=contset(opt,'VarTolerance',1e-3);
opt=contset(opt,'FunTolerance',1e-3);
opt=contset(opt,'MaxStepsize',0.01);
...
[x0,v0]=init_ZH_H(@RDPPSystemOnNetwork1,initial_xZH,p,ap1);
opt=contset;
opt=contset(opt,'VarTolerance',1e-3);
opt=contset(opt,'FunTolerance',1e-3);
opt=contset(opt,'MaxStepsize',0.001);
...
% [x0,v0]=init_ZH_LP(@RDPPSystemOnNetwork1,initial_xZH,p,ap1);
[x0,v0]=init_LP_LP(@RDPPSystemOnNetwork1,initial_xZH,p,ap1,[1 2]);
opt=contset;
